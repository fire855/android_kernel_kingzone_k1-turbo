...
# Vanzo:songlixin on: Tue, 01 Jul 2014 20:43:23 +0800
# make lk.bin partition to be writeable so ota can update it
=begin
...
=end
=cut
        if ($part->{Partition_Name} eq "PRELOADER" || $part->{Partition_Name} eq "DSP_BL" || $part->{Partition_Name} eq "SEC_RO")
