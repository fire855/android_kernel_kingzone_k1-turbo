...
#ifdef EXTTE_FOR_ESD_CHECK_SUPPORT
#include <mach/eint.h>
#include <cust_eint.h>
#endif
...
#ifdef EXTTE_FOR_ESD_CHECK_SUPPORT
/* will add them to response ext TE IRQ */
static bool extTeirq_flag = FALSE;

/* set flag TRUE when recieve ext TE */
static int DSI_ESD_wait_extTE_irq(bool en)
{
    printk("\n DSI_ESD_wait_extTE_irq[%d]", en);
    extTeirq_flag = en;
    return 0;
}

static int DSI_ESD_extTEirq_flag()
{
    return extTeirq_flag;
}

/* will add to  wait ext TE */
int DSI_extTe_esd_check(void)
{
    long ret;
    static const long WAIT_TIMEOUT = HZ/10;    // 100ms

    DSI_ESD_wait_extTE_irq(false);
    ...
    ret = wait_event_interruptible_timeout(_dsi_wait_ext_te, DSI_ESD_extTEirq_flag(), WAIT_TIMEOUT);
    /* if wait timeout, will return 1 and do esd recovery, here */
    if (0 == ret)
    {
        DISP_LOG_PRINT(ANDROID_LOG_WARN, "DSI", "Wait DSI_extTe_esd_check timeout!!!\n");
        //DSI_DumpRegisters();
        ///should we reset DSI
        DSI_Reset();
        return 1;
...
init_waitqueue_head(&_vsync_wait_queue);
    ...
    /* will register eint here */
#ifdef EXTTE_FOR_ESD_CHECK_SUPPORT
    mt_eint_registration(CUST_EINT_EXTTE_FOR_ESD_NUM, CUST_EINT_EXTTE_FOR_ESD_TYPE, DSI_ESD_wait_extTE_irq, 1); //the mask must be 1
#endif
    return DSI_STATUS_OK;
}
