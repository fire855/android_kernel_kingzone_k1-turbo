...
#ifdef EXTTE_FOR_ESD_CHECK_SUPPORT
/* will add this func to do esd check */
DISP_STATUS DISP_extTE_esd_check(void)
{
    long ret = 0;
    unsigned int esd_check_count = 0;

    printk("\nenter DISP_extTE_esd_check");

    while (esd_check_count < LCM_ESD_CHECK_MAX_COUNT)
    {
        if(DSI_extTe_esd_check())
        {
            DISP_EsdRecover();
            //DSI_extTe_esd_recover();
        }
        else
        {
            break;
    ...
    {
        esd_kthread_pause = TRUE;
        printk("DISP_extTE_esd_check[%d] will pause", esd_check_count);
    }
    return DISP_STATUS_OK;
}
#endif
