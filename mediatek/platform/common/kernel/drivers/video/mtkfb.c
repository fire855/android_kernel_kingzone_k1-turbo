...
#ifdef EXTTE_FOR_ESD_CHECK_SUPPORT //this macro is defined in ProjectConfig.mk by yourself
/* will add this thread instead of original esd flow  */
DEFINE_SEMAPHORE(sem_esd_check);

static int MTKFB_extTE_esd_kthread(void *data)
{
    MTKFB_LOG("enter MTKFB_extTE_esd_kthread()\n");

    for( ;; )
    {
        MTKFB_LOG("sleep start in MTKFB_extTE_esd_kthread()\n");
        msleep(2000);       //2s
        MTKFB_LOG("sleep ends in MTKFB_extTE_esd_kthread()\n");

        if(!esd_kthread_pause)
        {
            if(is_early_suspended)
            {
                MTKFB_LOG("is_early_suspended in MTKFB_extTE_esd_kthread()\n");
                continue;
            }
            if(down_interruptible(&sem_esd_check))
            {
                printk("[FB Driver] can't get semaphore in _DISP_ESD_CheckKThread()\n");
                continue;
            }
            /* esd check and recovery func */
            DISP_extTE_esd_check();
            up(&sem_esd_check);
            if (kthread_should_stop())
            {
                MTKFB_LOG("\n MTKFB_extTE_esd_kthread _should_stop");
                break;
            }
            MTKFB_LOG("\nMTKFB_extTE_esd_kthread ends");
        }
        else
        {
            MTKFB_LOG("\nesd pause");
    ...
    MTKFB_LOG("exit MTKFB_extTE_esd_kthread()\n");
    return 0;
}
#endif
...
/* Vanzo:yangzhihong on: Thu, 11 Sep 2014 16:16:24 +0800
...
static int mtkfb_pan_display_impl(struct fb_var_screeninfo *var, struct fb_info *info)
{
    ...
    int wait_ret = 0;
    int layer = FB_LAYER;
/* Vanzo:yangzhihong on: Thu, 11 Sep 2014 16:15:46 +0800
...
static int mtkfb_probe(struct device *dev)
{
    ...
    /* usng macro EXTTE_FOR_ESD_CHECK_SUPPORT to startup a new esd check thread  */
#ifdef EXTTE_FOR_ESD_CHECK_SUPPORT //this macro is defined in ProjectConfig.mk by yourself
        if(DISP_EsdRecoverCapbility())
        {
            esd_recovery_task = kthread_create(MTKFB_extTE_esd_kthread, NULL, "extTE_esd_check_kthread");
            ...
            else
            {
                ...
                //esd_recovery_pause(FALSE); //statrt up esd check
            }
        }
#else
