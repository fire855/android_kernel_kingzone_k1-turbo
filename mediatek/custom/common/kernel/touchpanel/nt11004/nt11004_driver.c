...
/*
static int tpd_proximity_init(void)
{
    int err = 0;
    ...
    else
    {
        ...
        hardwareinfo_set_prop(HARDWARE_ALSPS, "nt11004");
    }
...
static int tpd_button_event(u8 buf1, u8 buf2)
{
	...
	if((buf1 >> 3) == 0x1F)
	{
		...
		case 0x01:
			#ifdef TPD_HAVE_BUTTON_TYPE_COOR
                #if defined(HVGA)
                    tpd_down(0,120,530,0,0);
                #elif (defined(WVGA) || defined(CU_WVGA) || defined(CMCC_WVGA))
                    tpd_down(0,180,870,0,0);
                #else
                    tpd_down(0,180,920,0,0);
                #endif
			#else
		...
		case 0x02:
			#ifdef TPD_HAVE_BUTTON_TYPE_COOR
                #if defined(HVGA)
                    tpd_down(0,40,530,0,0);
                #elif (defined(WVGA) || defined(CU_WVGA) || defined(CMCC_WVGA))
                    tpd_down(0,60,870,0,0);
                #else
                    tpd_down(0,60,920,0,0);
                #endif
			#else
		...
		case 0x04:
			#ifdef TPD_HAVE_BUTTON_TYPE_COOR
                #if defined(HVGA)
                    tpd_down(0,200,530,0,0);
                #elif (defined(WVGA) || defined(CU_WVGA) || defined(CMCC_WVGA))
                    tpd_down(0,300,870,0,0);
                #else
                    tpd_down(0,300,920,0,0);
                #endif
			#else
		...
		case 0x08:
			#ifdef TPD_HAVE_BUTTON_TYPE_COOR
                #if defined(HVGA)
                    tpd_down(0,280,530,0,0);
                #elif (defined(WVGA) || defined(CU_WVGA) || defined(CMCC_WVGA))
                    tpd_down(0,420,870,0,0);
                #else
                    tpd_down(0,500,1030,0,0);
                #endif
			#else
...
static int __devinit tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	i2c_read_bytes(i2c_client, 0x86, &buf[0], 2);

	if(0x04!=buf[0])
	{
//		while(1)
		{
			printk("TP probe error\n");
		}
		tpd_load_status = 0;
		return -1; 
	}
