...
//unsigned short msg_coordinate_x[150] = {0};
//unsigned short msg_coordinate_y[150] = {0};
u8 msg_original_coordinate[200] = {0};
...
static void _DrvFwCtrlMsg22xxProgramEmem(EmemType_e eEmemType) // For MSG22XX
{
    ...
    {
        DBG("eEmemType = %d is not supportd for program e-memory.\n", eEmemType);
        return;
    }
...
//struct alsps_hw *hw = get_ap3216c_cust_alsps_hw();
    DBG("*** %s() ***\n", __func__);
    ...
    if (g_GestureWakeupFlag == 1)
    {
        ...
        {
       /* 
	printk("[guoyi] get ps state addr 0x%x\n", hw->get_ps_state);
	if (NULL != hw->get_ps_state && hw->get_ps_state() == 1){
...
//                    input_report_key(g_InputDevice, KEY_Z, 0);
		...
		/*		
		for(i = 0;i < msg_pointnum;i++)
		{
		        msg_coordinate_x[i] = (( pPacket[0 + (3 * i) + 2]) & 0xF0) <<4 | (( pPacket[1 + (3 * i) + 2]) & 0xFF);
				msg_coordinate_x[i]= (msg_coordinate_x[i]* TOUCH_SCREEN_X_MAX) / TPD_WIDTH;
		        msg_coordinate_y[i] = (( pPacket[0 + (3 * i) + 2]) & 0x0F) <<8 | (( pPacket[2 + (3 * i) + 2]) & 0xFF);
				msg_coordinate_y[i]=TOUCH_SCREEN_Y_MAX -(msg_coordinate_y[i] * TOUCH_SCREEN_Y_MAX) / TPD_HEIGHT;
   			 DBG("terry gesture point x = %d, y = %d\n",msg_coordinate_x[i],msg_coordinate_y[i]);
		}
		*/
		for(i = 0;i < 28;i++)
		{
		        msg_original_coordinate[i] = pPacket[i + 2];
    			DBG("terry gesture point msg_original_coordinate= %d\n",msg_original_coordinate[i]);
		}	
        }
        else
        {
...
static s32 _DrvFwCtrlUpdateFirmwareC33BySwId(u8 szFwData[][1024], EmemType_e eEmemType) // For MSG21XXA
{
...
static void _DrvFwCtrlUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    {
        nRetVal = _DrvFwCtrlUpdateFirmwareC33BySwId(g_FwData, EMEM_MAIN);
    }
    ...
    if (nRetVal == 0)
    {
        ...
        if (g_ChipType == CHIP_TYPE_MSG22XX)    
        {
            DrvPlatformLyrTouchDeviceResetHw(); // Reset HW after update firmware is finished.
...
/*
DBG("_gDwIicInfoData[0] = %c && _gDwIicInfoData[1] == %c\n", _gDwIicInfoData[0], _gDwIicInfoData[1]);
DBG("_gDwIicInfoData[2] == %c && _gDwIicInfoData[3] == %c\n", _gDwIicInfoData[2], _gDwIicInfoData[3]);
DBG("_gDwIicInfoData[4] == %c && _gDwIicInfoData[5] == %c\n", _gDwIicInfoData[4], _gDwIicInfoData[5]);
DBG("_gDwIicInfoData[6] == %c && _gDwIicInfoData[7] == %c\n", _gDwIicInfoData[6], _gDwIicInfoData[7]); 
*/    
...
//        nChipType = RegGet16BitValue(0x1ECC) & 0xFF;
        ...
        {
#ifdef CONFIG_UPDATE_FIRMWARE_BY_SW_ID
            return _DrvFwCtrlUpdateFirmwareC33BySwId(szFwData, EMEM_MAIN);
#else
...
//        mdelay(200);
    ...
    DmaAlloc();
#endif //CONFIG_ENABLE_DMA_IIC
	DBG("gesture nReportPacketLength = %d\n",nReportPacketLength);
    IicReadData(SLAVE_I2C_ID_DWI2C, &pPacket[0], nReportPacketLength);
#ifdef CONFIG_ENABLE_DMA_IIC
    DmaFree();
#endif //CONFIG_ENABLE_DMA_IIC
...
u8 DrvFwCtrlOpenGestureWakeup(u16 nMode)
{
    u8 szDbBusTxData[3] = {0};
    u32 i = 0;
    s32 rc;
