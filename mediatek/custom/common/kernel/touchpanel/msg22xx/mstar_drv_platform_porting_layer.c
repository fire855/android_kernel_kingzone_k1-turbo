...
static int _DrvPlatformLyrFingerTouchHandler(void *pUnUsed)
{
    ...
    do
    {
        ...
        DBG("===touch_event_handler, task running===\n");
...
void DrvPlatformLyrTouchDeviceRegulatorPowerOn(struct regulator *pReguVdd)
{
    s32 nRetVal = 0;

    DBG("*** %s() ***\n", __func__);
    
    nRetVal = regulator_set_voltage(pReguVdd, 2800000, 2800000);

    if (nRetVal)
    {
        DBG("Could not set to 2800mv.\n");
    }
    regulator_enable(pReguVdd);
...
void DrvPlatformLyrSetIicDataRate(struct i2c_client *pClient, u32 nIicDataRate)
{
    ...
    // TODO : Please FAE colleage to confirm with device driver engineer of customer for how to set i2c data rate on SPRD platform
    //sprd_i2c_ctl_chg_clk(pClient->adapter->nr, nIicDataRate); 
    //mdelay(100);
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_QCOM_PLATFORM)
    // TODO : Please FAE colleage to confirm with device driver engineer of customer
#elif defined(CONFIG_TOUCH_DRIVER_RUN_ON_MTK_PLATFORM)
