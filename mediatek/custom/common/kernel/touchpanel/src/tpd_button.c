...
/* Vanzo:maxiaojun on: Fri, 13 Jun 2014 16:45:52 +0800
 * Add TP Virtual Key interface for button vibrate feedback.
 */
#include "lcm_drv.h"
...
/* Vanzo:maxiaojun on: Fri, 13 Jun 2014 16:41:15 +0800
 * Add TP Virtual Key interface for button vibrate feedback.
 */
        {
            int i;
    if(tpd_keycnt > 0 && tpd_keys_dim[0][1] < lcm_params->height){
        printk("MAMAMA config tpd key !!! \n");
        for(i=0;i<tpd_keycnt;i++){
            tpd_keys_dim[i][0] = lcm_params->width / keycnt / 2 + (lcm_params->width / keycnt) * i;
            tpd_keys_dim[i][1] = lcm_params->height + lcm_params->width / keycnt / 2 + 20;
            tpd_keys_dim[i][2] = lcm_params->width / keycnt / 2;
            tpd_keys_dim[i][3] = lcm_params->width / keycnt / 2;
        }
    }
        }
// End of Vanzo:maxiaojun
}
/* Vanzo:maxiaojun on: Wed, 11 Jun 2014 21:43:10 +0800
 * Add TP Virtual Key interface for button vibrate feedback.
 */
void input_report_key_tp(struct input_dev *dev, unsigned int code, int value)
{
    int i;
    for(i=0;i<tpd_keycnt;i++){
        if(code == tpd_keys[i]){
            if(value == 1){
                ...
                input_report_abs(tpd->dev, ABS_MT_POSITION_X, tpd_keys_dim[i][0]);
                input_report_abs(tpd->dev, ABS_MT_POSITION_Y, tpd_keys_dim[i][1]);
