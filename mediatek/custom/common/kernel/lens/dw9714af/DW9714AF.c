...
static struct i2c_board_info __initdata kd_lens_dev={ I2C_BOARD_INFO("DW9714AF", 0x21)};
...
static int s4DW9714AF_ReadReg(unsigned short * a_pu2Result)
{
  ...
  //printk("%s line %d pBuff[0] 0x%x, pBuff[1] 0x%x\n",__func__,__LINE__,pBuff[0],pBuff[1]);
  if (i4RetValue < 0) 
  {
  ...
  *a_pu2Result = (((u16)(pBuff[0]&0x3f)) << 4) + (pBuff[1] >> 4);
...
static int s4DW9714AF_WriteReg(u16 a_u2Data)
{
  ...
  DW9714AFDB("[DW9714AF] g_sr %d, write %d puSendCmd[0] 0x%x puSendCmd[1] 0x%x\n", g_sr, a_u2Data,puSendCmd[0],puSendCmd[1]);
...
static int DW9714AF_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    g_pstDW9714AF_I2Cclient->addr = DW9714AF_VCM_WRITE_ID;
