...
static void lcm_get_params(LCM_PARAMS *params)
{
    ...
    //params->dsi.packet_size=256; //no info from lk: Usually the values are 256 / 512 / not used(remove)

    // Video mode setting
    //params->dsi.intermediat_buffer_num = 0;//because DSI/DPI HW design change, this parameters should be 0 when video mode in MT658X; or memory leakage

    params->dsi.PS=LCM_PACKED_PS_24BIT_RGB888;
    //params->dsi.word_count=720*3; //no info from lk: Usually values for this platform are 720*3 / not used(remove)

    params->dsi.vertical_sync_active				= 1;
    params->dsi.vertical_backporch					= 1;  // 4 --> value of the second built lcm lk
    params->dsi.vertical_frontporch					= 32; // 2 --> value of the second built lcm lk
...
LCM_DRIVER e19_k168a_yikuailai_nt35596_lcm_drv =
{
    .name			= "e19_k168a_yikuailai_nt35596",
