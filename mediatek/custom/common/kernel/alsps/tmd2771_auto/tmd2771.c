...
/* Vanzo:yangzhihong on: Fri, 31 Oct 2014 20:21:57 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
static void TMD2771_read_ps_offset();
static u16  TMD2771_cali_ps_value(u16 ps_value);
#endif
...
struct TMD2771_priv {
	...
	int			ps_cali;
/* Vanzo:yangzhihong on: Fri, 31 Oct 2014 20:13:35 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
    int         ps_offset;
    bool        is_ps_cali;
...
static int TMD2771_init_client(struct i2c_client *client)
{
	...
	databuf[0]= 0x92; //TMD2771_CMM_ENABLE;
	res = TMD2771_i2c_master_operate(client, databuf, 0x101, I2C_FLAG_READ);
	if(res < 0)
	{
		APS_LOG(" TMD2771_init_client r!\n");
		goto EXIT_ERR;
	}
	gChip_ID = databuf[0];
...
long TMD2771_read_ps(struct i2c_client *client, u16 *data)
{
	...
	APS_LOG("yucong TMD2771_read_ps ps_data=%d, low:%d  high:%d ChipID 0x%x\n", *data, buffer[0], buffer[1],gChip_ID);
...
/* Vanzo:yangzhihong on: Fri, 31 Oct 2014 20:43:11 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
    *data = TMD2771_cali_ps_value(*data);
#endif        
// End of Vanzo:yangzhihong
	return 0;    
...
/* Vanzo:songlixin on: Wed, 19 Mar 2014 10:42:22 +0800
 * interface for factory mode
 */
...
/*----------------------------------------------------------------------------*/
/* Vanzo:yangzhihong on: Fri, 31 Oct 2014 20:25:15 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
static void TMD2771_read_ps_offset()
{
    ...
    int cnt;
    printk("%s line %d \n",__func__,__LINE__);
    if(TMD2771_obj->is_ps_cali)
        return;

    for(cnt=0; cnt<5; cnt++)
    {
        TMD2771_read_ps(TMD2771_obj->client, &data);
    ...
    TMD2771_obj->ps_offset = sum;
    TMD2771_obj->is_ps_cali = TRUE;
    printk("%s line %d ps_offset %d\n",__func__,__LINE__,TMD2771_obj->ps_offset);
}
...
static u16 TMD2771_cali_ps_value(u16 ps_value)
{
    ...
    if(TMD2771_obj->is_ps_cali)
    {
        if(TMD2771_obj->ps_offset < atomic_read(&TMD2771_obj->ps_thd_val_low))
        {
            cali_value = ps_value;
        }
        else if(TMD2771_obj->ps_offset < atomic_read(&TMD2771_obj->ps_thd_val_high))
        {
            cali_value = (ps_value == 1023) ? 1023 : ((ps_value > atomic_read(&TMD2771_obj->ps_thd_val_low))?(ps_value - atomic_read(&TMD2771_obj->ps_thd_val_low)): 0);
        }
        else
        {
            cali_value = (ps_value == 1023) ? 1023 : ((ps_value > TMD2771_obj->ps_offset) ?(ps_value - TMD2771_obj->ps_offset):0);
        }
    }
...
/* Vanzo:yangzhihong on: Fri, 31 Oct 2014 20:23:32 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
                TMD2771_read_ps_offset();
#endif
// End of Vanzo:yangzhihong
				break;
...
/* Vanzo:yangzhihong on: Fri, 31 Oct 2014 20:24:23 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
    TMD2771_obj->is_ps_cali = FALSE;
    TMD2771_obj->ps_offset = 0;
#endif
// End of Vanzo:yangzhihong
	APS_LOG("%s: OK\n", __func__);
	TMD2771_init_flag = 0;
