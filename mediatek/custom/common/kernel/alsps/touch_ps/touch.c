...
/* drivers/hwmon/mt6516/amit/touch.c - touch ALS/PS driver
 *
...
/*----------------------------------------------------------------------------*/
//static int touch_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
//       unsigned long arg)
static long touch_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
...
static int touch_probe(struct platform_device *pdev)
{
    ...
    wake_lock_init(&ps_lock, WAKE_LOCK_SUSPEND, "touchps wakelock");
    APS_LOG("%s: OK\n", __func__);
...
static int __init touch_init(void)
{
	...
	printk("[czb]@touch_init success,drivername = [%s]\n",touch_alsps_driver.driver.name);
	return 0;
}
/*----------------------------------------------------------------------------*/
