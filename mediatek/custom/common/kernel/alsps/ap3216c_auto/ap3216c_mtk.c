...
/* Vanzo:yangzhihong on: Sat, 01 Nov 2014 16:56:58 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
static void AP3216C_psensor_calibration()
{
    u16 data,sum=0;
    int cnt, res;
    u8 databuf[2];
    ...
    for(cnt=0; cnt<5; cnt++)
    {
       AP3216C_read_ps(AP3216C_obj->client, &data);
       sum += data;
    ...
    sum = sum/5;
    printk("%s line %d sum %d\n",__func__,__LINE__,sum);

    if(sum > 511) sum =511;

   
	databuf[0] = 0x28;    //set the crosstalk 100
	databuf[1] = sum & 0x01;
	res = i2c_master_send(AP3216C_obj->client, databuf, 0x02);
	if(res <= 0)
	{
        printk("%s line %d\n",__func__,__LINE__);
		return AP3216C_ERR_I2C;
...
/* Vanzo:yangzhihong on: Sat, 01 Nov 2014 16:20:59 +0800
 */
#ifdef VANZO_ALSPS_AUTO_CALIBRATION
        case  ALSPS_IOCTL_GET_CALI:
            printk("%s line %d \n",__func__,__LINE__);
            AP3216C_psensor_calibration();
            break;
#endif
// End of Vanzo:yangzhihong
		default:
...
#endif
	wake_lock_init(&ps_lock, WAKE_LOCK_SUSPEND, "ap3216c wakelock");
