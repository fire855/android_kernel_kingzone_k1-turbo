...
static int PS_INTT 					= EPL_INTT_PS_150;//EPL_INTT_PS_90;
...
static struct sensor_init_info epl8865_init_info = {
        .name = "epl8865",
...
static ssize_t epl8865_store_ges_enable(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	GES_LOG("epl8865_gesture_store ges_enable %d\n",mode);
    if(mode)
...
#ifdef BUILD_CTS
static DRIVER_ATTR(gesture,   S_IRUGO, epl8865_gesture_show, epl8865_gesture_store);
static DRIVER_ATTR(ges_mode_state,   S_IRUGO, epl8865_show_ges_mode_state, epl8865_store_ges_mode_state);
#else
...
static long epl8865_unlocked_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        case ALSPS_SET_PS_MODE:
            ...
            if(enable)
            {
                mutex_lock(&sensor_mutex); //add  parker 2014/9/2
				set_bit(CMC_BIT_PS, &obj->enable);
				mutex_unlock(&sensor_mutex);//add  parker 2014/9/2
            }
            else
            {
                mutex_lock(&sensor_mutex); //add  parker 2014/9/2
    ...
    switch (command)
    {
        ...
        case SENSOR_GET_DATA:
            ...
            else
            {
                ...
                sensor_data->value_divide = 1;
                sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;
                APS_LOG("epl8865 get als data->values[0] = %d\n", sensor_data->values[0]);
            }
            break;
...
static int __init epl8865_init(void)
{
    ...
    hwmsen_alsps_sensor_add(&epl8865_init_info);
