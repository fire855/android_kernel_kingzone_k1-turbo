...
static int I2C_RxData(char *rxData, int length)
{
	...
	for(loop_i = 0; loop_i < QMC6983_RETRY_COUNT; loop_i++)
	{
		...
		printk(KERN_ERR "QMC6983 i2c_read retry %d times\n", QMC6983_RETRY_COUNT);
		mdelay(10);
	}
...
static int QMC6983_read_mag_xyz(int *data)
{
	...
	//databuf[0]=CTL_REG_ONE;
	//res=I2C_RxData(databuf,1);
	//MSE_LOG("QMC6983 CTL_ONE = after [%02X]  _b\n", databuf[0]);
...
static void qmc6983_work(struct work_struct *work)
{
	...
	//if (!qmc5983->enabled)
	//	goto out;
MSE_LOG("qmc6983_work, counter=%d\n", counter);
	counter++;
...
/* Get the Gain range */
/*
int QMC6983_get_range(char* range)
{
	int err = 0;
	char buf[1];
	buf[0]=CTL_REG_ONE;
...
/*
static void qmc6983_put_idle(struct i2c_client *client)
{
printk(KERN_ALERT "qmc6983_put_idle\n");
	...
	i2c_smbus_write_byte_data(client, MR_REG_M, QMC6983_IDLE_MODE);
}
*/
...
//		err = QMC6983_set_mode(*data);
	...
	case QMC6983_READ_MAGN_XYZ:
		...
		//MSE_LOG("QMC6983_wait A begin \n");
		//err = wait_for_completion_timeout(&data_updated, 2*HZ);
		//MSE_LOG("QMC6983_wait A Go......\n");
		//err = get_latest_mag_xyz(vec);
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	{
		case SENSOR_DELAY:
			printk(KERN_ERR "qmc6983 Set delay !\n");
...
static int __init qmc6983_init(void)
{
    ...
    hwmsen_msensor_add(&qmc6983_init_info);
	return 0;
}
/*----------------------------------------------------------------------------*/
