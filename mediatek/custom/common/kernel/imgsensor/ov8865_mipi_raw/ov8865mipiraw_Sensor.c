...
const kal_uint8 write_id_list[] = {0x6c, 0x20};
static kal_uint8 write_id;
static bool write_id_found = false;

/* #define OV8865_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, OV8865MIPI_WRITE_ID) */
#define OV8865_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, write_id)

kal_uint16 OV8865_read_cmos_sensor(kal_uint32 addr)
{
    kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte, write_id);
    return get_byte;
}
...
void OV8865VideoSetting(void)
{
	...
	OV8865_write_cmos_sensor(0x3809,0xc0);// ; X output size L  (0x3809,0xc0)
...
UINT32 OV8865GetSensorID(UINT32 *sensorID)
{
	...
	if (!write_id_found)		/* find the correct id */
	{
		for (i = 0, retry = 3; i < sizeof(write_id_list) / sizeof(kal_int8); i++)
		{
			write_id = write_id_list[i];
			OV8865_write_cmos_sensor(0x0103,0x01);// Reset sensor
			mDELAY(10);
			while(retry--)
			{
