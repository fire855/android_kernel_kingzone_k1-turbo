...
static int CAMERA_HW_Reg_Debug_sub( struct file *file, const char *buffer, unsigned long count,void *data)
{
	char regBuf[64] = {'\0'};
...
/* Vanzo:yangzhihong on: Thu, 26 Jun 2014 10:23:01 +0800
 * TODO: replace this line with your comment
 */
void CAMERA_HW_Get_sensor_name(char *sensorName)
{
     strcpy(sensorName, (char *)g_invokeSensorNameStr[0]);
}
EXPORT_SYMBOL(CAMERA_HW_Get_sensor_name);
...
static int __init CAMERA_HW_i2C_init(void)
{
   ...
   prEntry = create_proc_entry("driver/camsensorsub", 0, NULL);
		if (prEntry) {
					prEntry->read_proc = CAMERA_HW_DumpReg_To_Proc;
					prEntry->write_proc = CAMERA_HW_Reg_Debug_sub;
    }
		else {
        PK_ERR("add /proc/driver/camsensorsub entry fail \n");
    }
