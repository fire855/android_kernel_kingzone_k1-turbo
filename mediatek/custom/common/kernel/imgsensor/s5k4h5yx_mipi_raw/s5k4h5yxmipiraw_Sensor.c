...
kal_uint32 S5K4H5YX_SHUTTER_COUNT;
kal_uint32 s5k4h5yx_slave_addr = S5K4H5YXMIPI_WRITE_ID;
...
#define S5K4H5YX_write_cmos_sensor(addr, para) iWriteReg((u16) addr , (u32) para , 1, s5k4h5yx_slave_addr)
...
kal_uint16 S5K4H5YX_read_cmos_sensor(kal_uint32 addr)
{
    ...
    iReadReg((u16) addr ,(u8*)&get_byte,s5k4h5yx_slave_addr);
    return get_byte;
}
...
{   
	...
	{       
		...
		{           
			iBurstWriteReg(puSendCmd , tosend, s5k4h5yx_slave_addr);           
			tosend = 0;       
		}   
	}   
...
void S5K4H5YX_write_shutter(kal_uint32 shutter)
{
	...
	shutter_count = S5K4H5YX_SHUTTER_COUNT;
	S5K4H5YX_SHUTTER_COUNT = shutter;
...
void S5K4H5YXPreviewSetting(void)
{
	...
	if(chip_id == 0x01)
	{
		...
		S5K4H5YX_write_cmos_sensor(0x0346,0x00);
		S5K4H5YX_write_cmos_sensor(0x0347,0x06);
		S5K4H5YX_write_cmos_sensor(0x0348,0x0C);
		S5K4H5YX_write_cmos_sensor(0x0349,0xC9);
		S5K4H5YX_write_cmos_sensor(0x034A,0x09);
		S5K4H5YX_write_cmos_sensor(0x034B,0x99);
		S5K4H5YX_write_cmos_sensor(0x034C,0x06);
		S5K4H5YX_write_cmos_sensor(0x034D,0x60);
		S5K4H5YX_write_cmos_sensor(0x034E,0x04);
		S5K4H5YX_write_cmos_sensor(0x034F,0xc8);
		}
	...
	{ 
		...
		S5K4H5YX_write_cmos_sensor(0x3C1A,0xa8);//0xec//a8
...
void S5K4H5YXCaptureSetting(void)
{
	...
	{ 
		...
		S5K4H5YX_write_cmos_sensor(0x3C1E,0x00);
		S5K4H5YX_write_cmos_sensor(0x3500,0x0C);
		S5K4H5YX_write_cmos_sensor(0x3C1A,0xa8);//ec//a8
		S5K4H5YX_write_cmos_sensor(0x3002,0x03);
		S5K4H5YX_write_cmos_sensor(0x0200,0x0D);
		S5K4H5YX_write_cmos_sensor(0x0201,0x78);
		S5K4H5YX_write_cmos_sensor(0x300A,0x03);
...
UINT32 S5K4H5YXOpen(void)
{
    ...
    for(j=0;j<2;j++)
	{
		...
		if(S5K4H5YX_SENSOR_ID == sensor_id)
		  break;
		switch(j)
		{
			case 0:
				s5k4h5yx_slave_addr = S5K4H5YXMIPI_WRITE_ID2;
				break;
			case 1:
				s5k4h5yx_slave_addr = S5K4H5YXMIPI_WRITE_ID;
				break;
			default:
				break;
		}
...
UINT32 S5K4H5YXGetSensorID(UINT32 *sensorID)
{
	...
	{
		S5K4H5YXDB("sensor_id=%x\n",*sensorID);
    	if(S5K4H5YX_SENSOR_ID == *sensorID)
		  break;
		switch(j)
		{
			case 0:
...
if(0 == strncmp(VANZO_MAIN_CAM_ROTATION, "180", 3))	
	...
	S5K4H5YXSetFlipMirror(IMAGE_NORMAL);
