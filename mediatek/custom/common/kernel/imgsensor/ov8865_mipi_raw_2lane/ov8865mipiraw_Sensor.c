...
#define OV8865_DEBUG
#ifdef OV8865_DEBUG
	#define OV8865DB(fmt, arg...) printk(fmt, ##arg)
#else
...
static struct OV8865_SENSOR_STRUCT OV8865_sensor=
{
	.i2c_write_id = 0x6c,
	.i2c_read_id  = 0x6d,
};
...
int OV8865_write_cmos_sensor(u16 addr, u32 para)
{
	  return  iWriteReg((u16) addr , (u32) para , 1, OV8865_sensor.i2c_write_id);
}

kal_uint16 OV8865_read_cmos_sensor(kal_uint32 addr)
{
kal_uint16 get_byte=0;
    iReadReg((u16) addr ,(u8*)&get_byte,OV8865_sensor.i2c_write_id);
    return get_byte;
}
...
static int read_otp_wb(int index, struct otp_struct *otp_ptr)
{
	...
	OV8865DB("(*otp_ptr).light_rg =0x%x, (*otp_ptr).light_bg=0x%x\n",(*otp_ptr).light_rg, (*otp_ptr).light_bg);
	// clear otp buffer
...
static int update_otp_wb()
{
	...
	OV8865DB("current_otp.light_rg =0x%x, current_otp.light_bg=0x%x\n",current_otp.light_rg, current_otp.light_bg);
	//calculate G gain
	...
	G_gain = 0x400 * nG_G_gain / (nBase_gain);
		//before OTP update
	OV8865DB("before otp update reg[0x5018] = %x\r\n", OV8865_read_cmos_sensor(0x5018));
	OV8865DB("before otp update reg[0x5019] = %x\r\n", OV8865_read_cmos_sensor(0x5019));
	OV8865DB("before otp update reg[0x501a] = %x\r\n", OV8865_read_cmos_sensor(0x501a));
	OV8865DB("before otp update reg[0x501b] = %x\r\n", OV8865_read_cmos_sensor(0x501b));
	OV8865DB("before otp update reg[0x501c] = %x\r\n", OV8865_read_cmos_sensor(0x501c));
	OV8865DB("before otp update reg[0x501d] = %x\r\n", OV8865_read_cmos_sensor(0x501d));
	OV8865DB("RG_Ratio_Typical = %x\r\n", RG_Ratio_Typical);
	OV8865DB("BG_Ratio_Typical = %x\r\n", BG_Ratio_Typical);
	OV8865DB("RG = %x\r\n", rg);//rg
	OV8865DB("BG = %x\r\n", bg);//bg
	...
	update_awb_gain(R_gain, G_gain, B_gain);
	  ...
	  //after 
	OV8865DB("after otp update reg[0x5018] = %x\r\n", OV8865_read_cmos_sensor(0x5018));
	OV8865DB("after otp update reg[0x5019] = %x\r\n", OV8865_read_cmos_sensor(0x5019));
	OV8865DB("after otp update reg[0x501a] = %x\r\n", OV8865_read_cmos_sensor(0x501a));
	OV8865DB("after otp update reg[0x501b] = %x\r\n", OV8865_read_cmos_sensor(0x501b));
	OV8865DB("after otp update reg[0x501c] = %x\r\n", OV8865_read_cmos_sensor(0x501c));
	OV8865DB("after otp update reg[0x501d] = %x\r\n", OV8865_read_cmos_sensor(0x501d));
	return 0;
}
// call this function after OV8865 initialization
// return value: 0 update success
...
{
	OV8865DB(" OV8865videoSetting_2lane enter:video/preview sync\n");
...
void OV8865CaptureSetting(void)
{
	...
	OV8865_write_cmos_sensor(0x3106,0x21);//diff 4lane
	...
	OV8865_write_cmos_sensor(0x3729,0x02);//
	OV8865_write_cmos_sensor(0x372a,0x03);//
	OV8865_write_cmos_sensor(0x372b,0x50);//
	OV8865_write_cmos_sensor(0x372c,0xa3);//
	OV8865_write_cmos_sensor(0x372d,0x53);//
	OV8865_write_cmos_sensor(0x372e,0x06);//
	OV8865_write_cmos_sensor(0x372f,0x10);//
	...
	OV8865_write_cmos_sensor(0x373e,0x03);//
	OV8865_write_cmos_sensor(0x375a,0x06);//
	OV8865_write_cmos_sensor(0x375b,0x03);//
	OV8865_write_cmos_sensor(0x375d,0x02);//
	OV8865_write_cmos_sensor(0x375f,0x14);//
	OV8865_write_cmos_sensor(0x3767,0x04);//
	OV8865_write_cmos_sensor(0x3769,0x20);//
...
{
	OV8865DB("OV8865_Sensor_Init_2lane enter\n");
	...
	OV8865_write_cmos_sensor(0x0100,0x01);// ; wake up, streaming
  OV8865_write_cmos_sensor(0x5b00,0x02);//dpc control 
  OV8865_write_cmos_sensor(0x5b01,0xd0);//dpc control 
  OV8865_write_cmos_sensor(0x5b02,0x03);//dpc control 
  OV8865_write_cmos_sensor(0x5b03,0xcf);//dpc control 
  OV8865_write_cmos_sensor(0x5b05,0x6c);//dpc control 
  OV8865_write_cmos_sensor(0x5780,0xfc);//dpc control 
  OV8865_write_cmos_sensor(0x5781,0xdf);//dpc control 
  OV8865_write_cmos_sensor(0x5782,0x3f);//dpc control 
  OV8865_write_cmos_sensor(0x5783,0x08);//dpc control 
  OV8865_write_cmos_sensor(0x5784,0x0c);//dpc control 
  OV8865_write_cmos_sensor(0x5786,0x20);//dpc control 
  OV8865_write_cmos_sensor(0x5787,0x40);//dpc control 
  OV8865_write_cmos_sensor(0x5788,0x08);//dpc control 
  OV8865_write_cmos_sensor(0x5789,0x08);//dpc control 
  OV8865_write_cmos_sensor(0x578a,0x02);//dpc control 
  OV8865_write_cmos_sensor(0x578b,0x01);//dpc control 
  OV8865_write_cmos_sensor(0x578c,0x01);//dpc control 
  OV8865_write_cmos_sensor(0x578d,0x0c);//dpc control 
  OV8865_write_cmos_sensor(0x578e,0x02);//dpc control 
  OV8865_write_cmos_sensor(0x578f,0x01);//dpc control 
  OV8865_write_cmos_sensor(0x5790,0x01);//dpc control 
...
{
	int  retry = 3;
	printk("OV8865GetSensorID enter :\n ");
	...
	do {
		*sensorID = (OV8865_read_cmos_sensor(0x300B)<<8)|OV8865_read_cmos_sensor(0x300C);
		...
		{
			printk("Sensor ID1 = 0x%04x\n", *sensorID);
			break;
		}
		printk("Read Sensor ID Fail = 0x%04x line %d\n", *sensorID,__LINE__);
		retry--;
	} while (retry > 0);
    
	if (*sensorID != OV8865_SENSOR_ID) {
		OV8865_sensor.i2c_write_id = 0x20;
		OV8865_write_cmos_sensor(0x0103,0x01);// Reset sensor
		mDELAY(10);
		do {
			*sensorID = (OV8865_read_cmos_sensor(0x300B)<<8)|OV8865_read_cmos_sensor(0x300C);
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	switch (ScenarioId)
    {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            ...
            break;
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			OV8865DB("OV8865 video_preiew sync\n");
